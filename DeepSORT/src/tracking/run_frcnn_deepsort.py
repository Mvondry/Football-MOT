...
from src.detectors.faster_rcnn_detector import FasterRCNNDetector


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Faster R-CNN detector with DeepSORT and export MOT-format tracking results."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Device for Faster R-CNN inference, e.g. 'cuda' or 'cpu'.",
    )
...
def main() -> None:
    ...
    frcnn_weights = Path(detector_config["weights"])
    if not frcnn_weights.is_absolute():
        frcnn_weights = resolve_project_path(frcnn_weights)
    ...
    keep_labels = detector_config.get("keep_labels", None)

    detector = FasterRCNNDetector(
        weights_path=frcnn_weights,
        num_classes=int(detector_config["num_classes"]),
    ...
    print("=== Faster R-CNN + DeepSORT tracking ===")
    print(f"Experiment:          {experiment_name}")
    print(f"Tracker name:        {tracker_name}")
    print(f"Data root:           {data_root}")
    print(f"Split file:          {split_file}")
    print(f"FRCNN weights:       {frcnn_weights}")
